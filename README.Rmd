...
Default jumping is chosen based on default RStudio autocreation of matching parenthesis and quotes with combination of most common R code strings. It moves
maximum to the right in order to jump to a "reasonably furthest" destination.
...
You can define your one set of string to be jumped over. In 'cursorjumper' they
are called "barrier strings". Here are the default barrier strings (divided with ","):

```{r default-barrier-strings, echo = FALSE, eval = TRUE}
cat(paste0(cursorjumper::default_barrier_strings, collapse = ", "))
```

To set and get barrier strings there are `set_barrier_strings()` and `get_barrier_strings()` functions:

```{r set-get-barrier-strings}
...
There is also a "Jump Single Character" addin, which is an alternative to "Right" arrow keyboard button. It is useful if you want an alternative keyboard shortcut for it, which RStudio doesn't seem to support yet.
